...
export default {
  ...
  setup() {
    ...
    const getTableInfo = () => {
      ...
      getModelLocalDeploy(task_type, { username: username, is_public: 1 }).then(
        (res) => {
          console.log("res = ", res);
    ...
    const DownloadDeploy = (index, row) => {
      ...
      postModelLocalDeployDownload(
        ...
        qs.stringify({
          username: username,
          sharing_level: newShare.value,
          public_localdeployment_name: tableData.value[localIndex].deployname,
          public_localdeployment_owner: tableData.value[localIndex].owner,
        }),
      ).then((res) => {
        //console.log(res);
