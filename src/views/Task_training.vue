...
import { getTaskTrainingInfo, postTaskTrainingInfo } from "../api/Task_api";
import {
  getModelInfo,
  postModelCreate,
  postModelInfo,
...
export default {
  ...
  setup() {
    ...
    const getTableInfo = () => {
      ...
      getTaskTrainingInfo(task_type, { username: username }).then((res) => {
        console.log("res = ", res);
        ...
        for (var i = 0; i < length; i++) {
          tableData.value.push({
            ...
            task_id: obj[i].id,
            model: obj[i].model,
            dataset: obj[i].dataset,
            task_status: obj[i].status,
            optimizer: obj[i].optimizer,
            lr_scheduler: obj[i].lr_scheduler,
            batch_size: obj[i].batch_size,
            lr: obj[i].lr,
          });
        }
      });
    ...
    const handleDelete = (index, row) => {
      ...
      ElMessageBox.confirm("确定要删除吗？", "提示", {
        ...
        .then(() => {
          postTaskTrainingInfo(
            task_type,
            qs.stringify({
              task_id: tableData.value[index].task_id,
              username: username,
              delete: 1,
            }),
          ).then((res) => {
