...
//                     title: '手工标注数据（图像分割）'
//                 },
//                 component: () => import('/src/views/AnnotationForImageSegmentation.vue')
...
//                     title: '手工标注数据（文本分类）'
//                 },
//                 component: () => import('/src/views/AnnotationForTextClassification.vue')
//             }, { //AnnotationFortextcls
...
//                     title: '手工标注数据（视频分类）'
//                 },
//                 component: () => import('/src/views/AnnotationForActionRecognition.vue')
//             }, { //模型
//                 path: '/MyModel',
//                 name: 'MyModel',
//                 meta: {
...
//                     title: '测试模型'
//                 },
//                 component: () => import('/src/views/ModelTestingForImageClassification.vue')
//             }, { //模型测试-目标检测
...
//                     title: '测试模型'
//                 },
//                 component: () => import('/src/views/ModelTestingForObjectDetection.vue')
//             }, { //模型测试-目标跟踪
...
//                     title: '测试模型'
//                 },
//                 component: () => import('/src/views/ModelTestingForImageSegmentation.vue')
//             }, { //模型测试-文本分类
...
//                     title: '测试模型'
//                 },
//                 component: () => import('/src/views/ModelTestingForTextClassification.vue')
//             }, { //模型测试-视频分类
...
//                     title: '测试模型'
//                 },
//                 component: () => import('/src/views/ModelTestingForActionRecognition.vue')
//             },
//             { //Test
//                 path: '/Test',
//                 name: 'Test',
...
// 递归函数，用于遍历路由对象
async function downloadVueFiles(router, basePath = "") {
  if (router.children) {
    router.children.forEach((child) => {
      downloadVueFiles(child, basePath); // 递归遍历子路由
    });
  }

  if (router.component) {
    const componentPath = router.component.toString().match(/'(.*\.vue)'/);
    if (componentPath && componentPath[1]) {
      const vueFilePath = path.join(basePath, componentPath[1]);
      await downloadFile(vueFilePath);
...
function downloadFile(filePath) {
  const url = `https://shanhu.hpccube.com:50332${filePath}?raw`; // 替换为你的网站地址
  axios({
    url: url,
    method: "GET",
    // responseType: 'stream',
    ...
    .then((response) => {
      ...
      // 正则表达式匹配并提取 export default "..." 内的内容
      const fullRegex = /export default "(.*?)"$/s;

      const fullMatch = response.data.match(fullRegex);
      if (fullMatch && fullMatch[1]) {
        // 移除转义字符和双引号
        // 将匹配到的内容中的转义字符替换为正常字符
        const vueContent = fullMatch[1]
          .replace(/\\"/g, '"')
          .replace(/\\r\\n/g, "\n")
          .replace(/\\n/g, "\n");
